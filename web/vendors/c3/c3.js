...
Axis.prototype.generateTickValues = function generateTickValues(values, tickCount, forTimeSeries) {
    ...
    if (tickCount) {
        ...
        // Calcular los checks segun el usuario
        if (targetCount === 1) {
            tickValues = [values[0]];
...
function Chart(config) {
    ...
    // vincular "esto" a la API anidada
...
// MEMO: clipId debe ser único porque entra en conflicto cuando existen múltiples gráficos    $$.clipId = "c3-" + +new Date() + '-clip', $$.clipIdForXAxis = $$.clipId + '-xaxis', $$.clipIdForYAxis = $$.clipId + '-yaxis', $$.clipIdForGrid = $$.clipId + '-grid', $$.clipIdForSubchart = $$.clipId + '-subchart', $$.clipPath = $$.getClipPath($$.clipId), $$.clipPathForXAxis = $$.getClipPath($$.clipIdForXAxis), $$.clipPathForYAxis = $$.getClipPath($$.clipIdForYAxis);
    ...
    $$.axes.subx = d3.selectAll([]); // necesidades al excluir subchart.js
};
...
c3_chart_internal_fn.initWithData = function (data) {
    ...
    // Establecer objetivos para ocultar si es necesario
    if (config.data_hide) {
    ...
    // cuando calibre, ocultar  // La clase TODO: arreglar
    if ($$.hasType('gauge')) {
        config.legend_show = false;
    }

    // Tamaños y escalas iniciales
    $$.updateSizes();
    $$.updateScales();

    // Establecer dominios para cada escala
    ...
    // Guarde el dominio x original para la actualización de zoom
    $$.orgXDomain = $$.x.domain();

    // Establecer escalas inicializadas para cepillar y hacer zoom
    if ($$.brush) {
        $$.brush.scale($$.subX);
    }
    ...
    // Definir g para el área del gráfico
    ...
    // Cobertura entera con rectos para eventos
    $$.initEventRect();
